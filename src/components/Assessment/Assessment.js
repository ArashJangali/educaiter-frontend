...
const Assessment = () => {
  ...
  useEffect(() => {
    if (topic && level && language && triggerFetch) {
      setResult("");
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (
            error.response?.data.message === "No credits left. Please upgrade."
          ) {
  ...
  }, [topic, level, language, triggerFetch]);

  const submitAnswer = () => {
    // Evaluate the answer on the server
    axios
      .post(
        `/assessment/evaluateAnswer/${language}/${topic}/${level}`,
      ...
      {apiLimitReached || noAccess ? (
        ...
        <div className="assessment-container">
          <div className="topic-selection">
            ...
            />
            <button
              disabled={!topic || !level || !language}
              className="fetch-question-btn"
          ...
          {!topic && (
            ...
            >
              ...
              />
              <div className="question-section">
                <p className="question">Please select a topic.</p>
              </div>
            </div>
          )}
